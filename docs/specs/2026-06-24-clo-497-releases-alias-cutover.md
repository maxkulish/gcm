...
- [ ] **AC-3 (FR-42):** The released binary runs on macOS and Linux: `gcm --version` prints the build-stamped version and `gcm --help` exits 0. (CI-provable for the runner's own target; the cross-OS human install is the checkpoint in §5.)
...
- [ ] **AC-6 (FR-44):** The guide documents that rollback is a **single-line** `~/.zshrc` alias revert, and explicitly instructs the user to **leave `/opt/script/git-commit-ai.sh` in place** so the revert target still exists.
