...
- Parse git path output from NUL-delimited fields (`-z`) with `-c core.quotePath=false`, so unicode/space/quote/newline filenames and the acceptance unicode-named file are handled correctly (FR-31 basic). On an unborn branch (no `HEAD`), combine the unstaged (`git diff`) and staged (`git diff --cached`) diffs rather than diffing against the empty tree. (The empty-tree object `4b825dc642cb6eb9a0fa8e4ced7b1d4154961131` does not exist in a fresh repo with no objects, so `git diff <empty-tree>` errors with "bad object"; the unstaged+staged combination captures every tracked change, including a staged-then-modified file, without depending on that object - revised after Codex+Gemini validation.)
...
## Post-implementation validation (Codex GPT-5.4 + Gemini 3.1 Pro)

A code-level review gate ran after implementation. Both returned FAIL with overlapping findings; the legitimate ones were fixed (one bogus "branch deleted" finding was the Gemini validator's own YOLO-mode `git` commit, reverted). Reports: `docs/reviews/clo-486-codex-validation.md`, `docs/reviews/clo-486-gemini-validation.md`.

- **Untracked read unbounded** (Codex/Gemini HIGH) - `diff.rs` now reads at most a per-file byte budget via `read_capped` (a single huge file is never fully loaded), counts every untracked path (text/binary/unreadable) toward the 50-file cap, and lists every beyond-cap file by name only with no read.
- **Index snapshot too late** (Codex/Gemini) - `main.rs` now takes the `write-tree` snapshot before gathering/prompting and restores on any post-snapshot failure, so the restore point is the true pre-run index.
- **Unborn-branch diff** (Codex/Gemini) - switched from the empty-tree object (absent in fresh repos) to unstaged+staged combination (see Constraints); AC-14b added.
- **build.rs rerun trigger** (Gemini LOW) - added `.git/logs/HEAD` so the SHA re-stamps on new commits to the same branch.
- **Test coverage** - acceptance.sh gained PTY-driven AC-2 abort, mock HTTP 500 (AC-12b) and empty-response (AC-12c) cases, AC-14b (staged-then-modified on unborn), and a corrected AC-4 cap count (50 content reads of 2000 files). 14 unit tests, 32 acceptance checks pass.
- **Not adopted:** Codex's "use `git log --show-signature`" for AC-1 - it gives a false negative without an `allowedSignersFile` (SSH signatures can't be verified there); the `gpgsig`-header check is strictly more reliable and is retained.
